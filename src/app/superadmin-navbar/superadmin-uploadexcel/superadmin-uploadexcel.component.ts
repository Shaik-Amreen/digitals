...
@Component({
  selector: 'app-superadmin-uploadexcel',
  templateUrl: './superadmin-uploadexcel.component.html',
  styleUrls: ['./superadmin-uploadexcel.component.css']
})
export class SuperadminUploadexcelComponent implements OnInit {
 ...
 saveu=false
  ...
  save(){
    ...
    for(let c of this.mapping){
      ...
      c.role=[];c.role.push(this.role);
      c.mail=c.mail.toLowerCase()
      c.password=c.mail;
    }
   
    this.http.post<any>('http://localhost:4000/users/registerarr',this.mapping).subscribe(
      res=>{
        this.saveMode=true;
       console.log(res)
